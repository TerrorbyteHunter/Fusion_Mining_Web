...
import pg from 'pg';
// Create our own pool so this script works in ESM without relying on compiled server/db.js
const { Pool } = pg;
const connectionString = process.env.DATABASE_URL || process.env.POSTGRES_URL || process.env.POSTGRES_CONNECTION_STRING;
if (!connectionString) {
